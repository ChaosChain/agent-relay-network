...
func main() {
	...
	db1 := badger.BadgerBackend{Path: "/tmp/khatru-badgern-tmp1"}
	if err := db1.Init(); err != nil {
		panic(err)
	}

	db2 := badger.BadgerBackend{Path: "/tmp/khatru-badgern-tmp2"}
	if err := db2.Init(); err != nil {
		panic(err)
	}

	db3 := badger.BadgerBackend{Path: "/tmp/khatru-badgern-tmp3"}
	...
	// when I do control+c, the program exits, but the tmp files are not cleared
	// so I need to clear them manually

	// wait for 10 seconds
	time.Sleep(10 * time.Second)

	// clear tmp files after program exits
	os.RemoveAll("/tmp/khatru-badgern-tmp1")
	os.RemoveAll("/tmp/khatru-badgern-tmp2")
	os.RemoveAll("/tmp/khatru-badgern-tmp3")
}
